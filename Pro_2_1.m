...
%X变成一维向量，
X_flat = X(:);
objectiveFunction = @(X_flat) obj(X_flat,P,S,C,A,num_plots, num_crops, num_years);
...
function Z = obj(X_flat,P,S,C,A,num_plots, num_crops, num_years)
    Z = 0;
    punish = 0;%惩罚度
    ...
    for i = 1:num_plots
        for j = 1:num_crops
            for k = 1:num_years
                Z = Z + (P(i,j,k).*S(i,j,k)-C(i,j,k)).*X(i,j,k); 
            end    
        end
    end
